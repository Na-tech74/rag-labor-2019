...
| 4. Retrieve | `src/retriever.py` | Vector search top-k chunks liên quan |
...
| rank-bm25 | Hybrid search (keyword) |
...
├── ingest.txt                 # Hướng dẫn OCR chi tiết
├── architecture.txt           # Kiến trúc hệ thống
└── structure.txt              # Cấu trúc thư mục
```
...
Các file `src/ocr.py`, `src/chunker.py`, `src/indexer.py`, `src/retriever.py` đang hardcode đường dẫn tuyệt đối `D:\rag-labor-2019\...`, và `ocr.py` hardcode đường dẫn `tesseract.exe`.
...
Chỉ xem top-k chunks mà không gọi LLM:
